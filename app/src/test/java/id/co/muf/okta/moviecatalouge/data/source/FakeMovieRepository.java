    ...
    public LiveData<Resource<MovieEntity>> getMovieDetail(final String imageId) {
        return new NetworkBoundResource<MovieEntity, List<MovieResponse>>(appExecutors) {
            ...
            protected Boolean shouldFetch(MovieEntity movieEntity) {
                return (movieEntity == null || movieEntity.getDescription() == null);
            }

            @Override
            protected LiveData<ApiResponse<List<MovieResponse>>> createCall() {
                return remoteRepository.getAllMovieDetailAsLiveData();
            }

            @Override
            protected void saveCallResult(List<MovieResponse> movieResponses) {
                localRepository.getDetailMovies(imageId);
//                List<MovieEntity> movieEntities = new ArrayList<>();
//
//                for (MovieResponse moduleResponse : movieResponses) {
//                    movieEntities.add(new MovieEntity(moduleResponse.getImageId(), moduleResponse.getTitle(), moduleResponse.getDescription(), moduleResponse.getDate(), moduleResponse.isFavorites(),moduleResponse.getImagePath()));
//                }
//
//                localRepository.insertMovies(movieEntities);
            }
    ...
    public LiveData<Resource<TvShowEntity>> getTvshowDetail(final String imageId) {
        return new NetworkBoundResource<TvShowEntity, List<TvshowResponse>>(appExecutors) {
            ...
            protected Boolean shouldFetch(TvShowEntity tvShowEntity) {
                return (tvShowEntity == null || tvShowEntity.getImageId() == null);
            }
            ...
            protected void saveCallResult(List<TvshowResponse> tvshowResponses) {
                localRepository.getDetailTvshows(imageId);
//                List<TvShowEntity> tvShowEntities = new ArrayList<>();
//
//                for (TvshowResponse moduleResponse : tvshowResponses) {
//                    tvShowEntities.add(new TvShowEntity(moduleResponse.getImageId(), moduleResponse.getTitle(), moduleResponse.getDescription(), moduleResponse.getDate(), moduleResponse.isFavorites(), moduleResponse.getImagePath()));
//                }
//
//                localRepository.insertTvshows(tvShowEntities);
            }
